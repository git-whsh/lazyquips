...
공개 build 및 test 명령은 의도적으로 code signing을 비활성화합니다. 이 명령은 소스 컴파일과 일반 테스트만 검증합니다. sandbox runtime behavior, login item registration, Developer ID signing, notarization, App Store export, TestFlight upload 또는 공식 release process는 검증하지 않습니다.
